...
EthRPC.prototype.eth_getStorage = function(params, cb) {
  var address = normalizeHexString(params[0]);
  var blockHash = normalizeHexString(params[1]);
  var self = this;
...
EthRPC.prototype.eth_getCount = function(params, cb) {
  var address = normalizeHexString(params[0]);
  var blockHash = normalizeHexString(params[1]);
  var self = this;
...
EthRPC.prototype.eth_signedTransact = function(params, cb) {
  ...
  this.app.vm.runTx({
    ...
    if (result.createdAddress) {
      results = +hexStrPrefix + result.createdAddress.toString('hex');
    }

    cb(null, hexStrPrefix);
  });
};
...
function fnForMethod(method) {
  var map = {
    ...
    eth_getStorage: EthRPC.prototype.eth_getStorage, //should be called dump storage
    eth_getCount: EthRPC.prototype.eth_getCount, // count aka nonce
