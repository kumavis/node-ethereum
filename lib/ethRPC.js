...
module.exports = EthRPC = function (opts) {
  ...
  this.app.vm.on('logs', function(log){
    //check each filter
    self.topics.forEach( function(t){
      //each topic in each log
      for(var topic in t){
        //check if the topic is in the bloom
        if(log.bloom.check(t)){
...
EthRPC.prototype.eth_unistallFilter = function(params, cb) {
  ...
  if(index !== -1) self.topics.splice(index, 1);
  cb();
};
