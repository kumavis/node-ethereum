...
function getBalance(params, cb) {
  var address = ethUtil.stripHexPrefix(params[0]);
...
function _signedTransact(params, cb) {
  ...
  var hexString = ethUtil.stripHexPrefix(params[0])
...
function getBlockByHash(params, cb) {
  var hash = ethUtil.stripHexPrefix(params[0])
...
function getTransactionByBlockNumberAndIndex(params, cb) {
...
function newFilter(params, cb) {
  ...
  .map(function(t){
    return ethUtil.stripHexPrefix(t);
  })

  if (filter.address) topics.push(ethUtil.stripHexPrefix(filter.address));
  self.logFilters[id] = {
    topics: topics,
    results: [],
  };
