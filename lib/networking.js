...
module.exports = function (done, results) {
  ...
  this.network.on('hello', function (hello, peer) {
    ...
    if (self.detailsDB.isOpen()) {
      self.detailsDB.get('peers', function(err, peers) {
        if(err || !peers[hello.id]){
          var peers = peers ? peers : {};
          peers[hello.id] = hello;
          self.detailsDB.put('peers', peers, function(err) {
            if (!err)
              console.log('Added Peer to List');
          });
        }
      }); 
    }
