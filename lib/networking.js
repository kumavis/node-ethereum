...
module.exports = function (done, results) {
  ...
  this.network.on('message.newBlock', function (message, peer) {
    log.info('networking', peer.internalId + ' got new Block:' + message.block.hash().toString('hex'));
    if (self.isSyncing) {
      blockQueue.push(message);
    }else{
      //TODO: check td
      self.processBlocks([message.block]);
    }
  });
