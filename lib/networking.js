...
module.exports = function(done, options) {
  ...
  var syncManager = new SyncManager(this);

  this.network.on('connection', function(peer) {
    log.info('connection!');
    if (peer.caps.eth) {
      ...
      peerMan.on('status', function(status) {
        ...
        if (self.blockchain.meta.genesis !== status.genesisHash.toString('hex')) {
          peer.sendDisconnect(peer.DISCONNECT_REASONS.SUBPROTOCOL_REASON);
        }else{
          syncManager.sync(peerMan);
        }
      });
      peerMan.sendStatus(new Buffer([]), utils.intToBuffer(self.blockchain.td), self.blockchain.head.hash(), new Buffer(self.blockchain.meta.genesis, 'hex'));
    }
  });
