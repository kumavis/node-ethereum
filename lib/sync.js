...
function syncBlocks(peer, source, cb) {
  ...
  async.whilst(function() {
    ...
    peer.eth.fetchBlocks(hashesToGet, function(blocks) {
      blocks.forEach(function(block) {
        var blockHash = block.hash().toString('hex');

        //removes the fetched hashes
      ...
      self.blockProcessor.run(blocksToProcess, function(err) {
        if (err && source) {
          peer.sendDisconnect();
          done = true;
...
function syncHashes(peer, startHash, cb) {
  ...
  async.whilst(function() {
    ...
    peer.eth.fetchBlockHashes(startHash, count, function(hs) {
...
sync = module.exports = function(peer, cb) {

  var td = new BN(peer.td);

  //get largest td that we have found
  if (largestTD.cmp(this.blockchain.td) === -1) {
    largestTD = new BN(this.blockchain.td);
  }

  if (largestTD.cmp(td) === -1) {
    ...
    stopSyncing = syncHashes.bind(this)(peer, peer.bestHash, cb);
  } else if (this.isSyncing) {
    //join the blockchain downloading effort
    syncBlocks(peer, false, cb);
  }
};
