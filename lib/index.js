...
App.prototype.start = function(cb) {
  ...
  function connectPeers(done, results) {
    results.loadPeers.forEach(function(peer) {
      self.network.connect(peer.value.ip, peer.value.port, function(err) {
        if (!err){
          log.info('networking','Connecting to '+ peer.value.ip+':'+ peer.value.port);
        }
        // If Peer isn't Connecting Evaluate it's Reputation
        else
          evaluatePeer(peer)
      });
    });
  ...
  function evaluatePeer(peer) {
    // If Peer Reputation is still more than 1 it loses it's rep
    if(peer.value.rep >= 1) {
      self.peersDB.put(peer.id, JSON.stringify({ip: peer.value.ip, port: peer.value.port, rep: --peer.value.rep}), function(err) {
        if(err)
    ...
    // If Peer Rep had 0, it get's deleted
      ...
      self.peersDB.del(peer.id, function(err) {
        if(err)
          log.error('database', err);
        else
