...
App.prototype.start = function(cb) {
  ...
  function evalGoodPeer(peer) {
    // If Peer Rep is less than Max Rep, it gains a rep
    if(peer.value.rep <= self.maxrep){
      self.peersDB.put(peer.id, JSON.stringify({ip: peer.value.ip, port: peer.value.port, rep: ++peer.value.rep}),
        function(err) {
          if(err)
    ...
    // If Peer Rep is at Max Rep, ...Nothing Happens
    else {
      log.info('networking','Connecting to '+ peer.value.ip+':'+ peer.value.port, 'At Max Rep');
    }
  }

  function evalBadPeer(peer) {
    // If Peer Reputation is still more than 1 it loses it's rep
    if(peer.value.rep >= 1) {
      self.peersDB.put(peer.id, JSON.stringify({ip: peer.value.ip, port: peer.value.port, rep: --peer.value.rep}),
        function(err) {
          if(err)
            ...
            log.info('networking', 'Peer '+peer.value.port+' Not Responding...Downvoting')
        })
    }
    // If Peer Rep had 0, it get's deleted
    else {
  ...
  function startXhrRpcServer(done) {
    self.xhrRpc = new XhrRpc({app: this});
    self.xhrRpc.start({}, done);
  }

  // WS RPC Server
  function startWsRpcServer(done) {
    self.wsRpc = new WsRpc({app: this});
