...
App.prototype.start = function(cb) {
  ...
  function connectPeers(done, results) {
    ...
    results.loadPeers.forEach(function(peer) {
      self.network.connect( peer.value.port, peer.value.ip,function(err) {
  ...
  if (this.settings.network) {
    tasks.network = ['ip', 'upnp', 'id', 'loadPeers', networking.bind(self)];
    tasks.connectPeers = ['network', connectPeers];
