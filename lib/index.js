...
App.prototype.start = function(cb) {
  ...
  function loadPeers(done) {
    ...
    self.peersDB.createReadStream()
      .on('data', function(peer) {
        peers.push(JSON.parse(peer.value));
      })
      .on('error', done)
      .on('end', function() {
        done(null, peers);
  ...
  function connectPeers(done, results) {
    results.loadPeers.forEach(function(peer) {
      self.network.connect(peer.ip, peer.port, function() {
  ...
  var tasks = {
    ...
    connectPeers: ['loadPeers', connectPeers],
    ip: noop,
    upnp: noop,
    rcp: noop,
    id: ['setup', getId],
  ...
  if(this.settings.dbServer){
    tasks.startDB = function startDBserver(done){
      dbServer = cp.fork('./bin/dbServer', ['--path', self.settings.path, '--port', self.settings.db.port], {execArgv: []} );
